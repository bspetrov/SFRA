...
function getAllCategories(product) {
    if (product) {
        try {
            ...
            Logger.error('Error in getAllCategories function in productBundle model: {0}', e.toString());
        }
    }
    return [];
}
...
function getSubcategory(categories, primaryCategory) {
    if (categories.length && primaryCategory) {
        try {
            ...
            Logger.error('Error in getSubcategory function in productBundle model: {0}', e.toString());
        }
    }
    return null;
}
...
module.exports = function bundleProduct(product, apiProduct, options, factory) {
    ...
    if (apiProduct) {
        try {
            ...
            Logger.error('Error in productBundle model: {0}', e.toString());
        }
    }
